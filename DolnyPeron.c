...
int s_krzeselka_count;     // semafor do kontroli liczby jednocześnie „aktywnych krzesełek”
int N = 20;
...
void* wjazd_krzeselek(void* args) {
    ...
    // z 3 miejsc w krześle (jeśli != 0)
    // printf("T1:%d T2:%d T3:%d\n", t->tablica[0], t->tablica[1],  t->tablica[2]);
...
int main() {
    ...
    while (!(adres2[0] == -1 && adres[3] < 1)){
        ...
        //usuwamy z peronu bo wsiadaja do krzeselek
        semafor_p(s_peron, 0);
        adres[3] -= grupa;
        semafor_v(s_peron, 0, 1);
        ...
        // Zanim wystartujemy wątek, „rezerwujemy” krzesło do max 40
